...
<?php
/*
Template Name: 2018 Results Page
*/
...
jQuery(document).ready( function () {
	...
	jQuery('#senators-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false, "columns": [ null, null, null, null, null, null, null, null,  null, null, null, null, null, { "type": "html" }, null, null, null, null, null, null, null, null  ] }    ); 
	jQuery('#scandidates-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false  }    );  
	jQuery('#representatives-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false, "order": [[ 3, 'asc' ], [ 2, 'asc' ]]  }   );
	jQuery('#candidates-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false, "order": [[ 3, 'asc' ], [ 2, 'asc' ]]  }    ); 	
...
<?php  
  ...
  $districtList = strip_tags(trim($_GET["district"])); 
  $zipcode = strip_tags(trim($_GET["zipcode"])); 
  $districts = array(); 
  ...
  if (!empty($zipcode) ) {
   ...
   $place = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}find_district WHERE zipcode=" . $zipcode  );
   $state = $place->state; 
   $districts = explode('|', $place->district);   
 }
 else if (!empty($districtList)) {
	 $districts = array( $districtList );
 }
  /* by now we have valid state and districts if possible */
...
?>
		...
		if ( post_password_required() ) {
			echo get_the_password_form();
						...
						if (!empty($state)) {
							$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}senate_2018 WHERE state='" . $state . "' ");
						}
						else {
						...
						if (!empty($state)) {
							$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}senate_candidates_2018 WHERE state='" . $state . "' ");
						}
						else {
						...
						$candidates = array();
							...
							if (!empty($state) && !empty($districts)) {																	

								$districtString = '(DISTRICT =';
								$i = 0;
							 	foreach( $districts as $district) {
									if ($i > 0) {
										$districtString .= ' OR district=';
									}
									$districtString .= $district;
									++$i;
								...
								$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}house_representatives_2018 WHERE state='" . $state . "' AND " . $districtString );
							}
							else if (!empty($state)) {
								$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}house_representatives_2018 WHERE state='" . $state . "' ");
							}
							else {
						...
						$candidates = array();
							...
							if (!empty($state) && !empty($districts)) {																	
								...
								$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}house_candidates_2018 WHERE state='" . $state . "' AND " . $districtString );
							}
							else if (!empty($state)) {
								$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}house_candidates_2018 WHERE state='" . $state . "' ");
							}
							else {
