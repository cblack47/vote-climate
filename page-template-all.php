...
jQuery(document).ready( function () {
	...
	jQuery('#senators-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false, "order": [[ 11, 'desc' ]]  }    ); 
	jQuery('#scandidates-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false, "order": [[ 7, 'desc' ]]  }    );
	jQuery('#representatives-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false, "order": [[ 11, 'desc' ]]  }   );
	jQuery('#candidates-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false, "order": [[ 8, 'desc' ]]  }    ); 	
...
?>
		...
		if ( post_password_required() ) {
			echo get_the_password_form();
						...
						$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}senate ");
						...
						$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}senate_candidates ");
						...
						$candidates = array();
							...
							$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}house_representatives");
						...
						$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}house_candidates ");
