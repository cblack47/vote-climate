...
jQuery(document).ready( function () {
	...
	jQuery('#senators-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false }  ); 
	jQuery('#scandidates-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false  }   );  
	jQuery('#representatives-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false, "order": [[ 1, 'asc' ], [ 2, 'asc' ]]  }   );
	jQuery('#candidates-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false, "order": [[ 1, 'asc' ], [ 2, 'asc' ]]  }    ); 	
...
?>
		...
		if ( post_password_required() ) {
			echo get_the_password_form();
						...
						if (!empty($state)) {
							$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}senate WHERE state='" . $state . "' ");
						}
						else {
						...
						if (!empty($state)) {
							$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}senate_candidates WHERE state='" . $state . "' ");
						}
						else {
						...
						$candidates = array();
							...
							if (!empty($state) && !empty($districts)) {																	
								...
								$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}house_representatives WHERE state='" . $state . "' AND " . $districtString );
							}
							else if (!empty($state)) {
								$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}house_representatives WHERE state='" . $state . "' ");
							}
							else {
						...
						$candidates = array();
							...
							if (!empty($state) && !empty($districts)) {																	
								...
								$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}house_candidates WHERE state='" . $state . "' AND " . $districtString );
							}
							else if (!empty($state)) {
								$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}house_candidates WHERE state='" . $state . "' ");
							}
							else {
