...
jQuery(document).ready( function () {
	...
	jQuery('#senators-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false, "order": [[ 0, 'asc' ]]  }    ); 
	jQuery('#scandidates-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false, "order": [[ 0, 'asc' ]]  }    );
	jQuery('#representatives-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false, "order": [[ 0, 'asc' ]]  }   );
	jQuery('#candidates-table').DataTable(  {"responsive": true, "paging": false, "info": false, "searching": false, "order": [[ 0, 'asc' ]]  }    ); 	
...
?>
		...
		if ( post_password_required() ) {
			echo get_the_password_form();
						...
						echo '<h1>Climate Zeros in the U.S. Senate</h1>';
												
						$candidates = array();
						
						$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}senate WHERE overallScore <= 1");
						...
						echo '<h1>Climate Zeros Running for the U.S. Senate</h1>';
						
						$candidates = array();
						
						$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}senate_candidates WHERE OverallScore <= 1");
						...
						echo '<h1>Climate Zeros in the U.S. House</h1>';
						
						$candidates = array();
						
							$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}house_representatives WHERE OverallScore <= 1");
						...
						echo '<h1>Climate Zeros Running for U.S. House</h1>';
						
						$candidates = array();
						
						$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}house_candidates WHERE OverallScore <= 1");
