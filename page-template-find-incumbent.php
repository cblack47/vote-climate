...
<?php
/*
Template Name: Find Incumbent Page
*/
  ...
  ?>
		...
		if ( post_password_required() ) {
			echo get_the_password_form();
						...
						if (!empty($name)) {
							$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}senate_scorecard WHERE Name LIKE '" . $name . "' ");
						}
						else {
						...
						if (!empty($name)) {
							$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}senate_scorecard_new WHERE Name LIKE '" . $name . "' ");
						}
						...
						echo '<h1>117th Representatives</h1>';
							...
							if (!empty($name)) {
								$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}house_scorecard WHERE Name LIKE '" . $name . "' ");
							}
						...
						$candidates = array();
						if (!empty($name)) {
							$candidates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}house_scorecard_new WHERE Name LIKE '" . $name . "' ");
						}
